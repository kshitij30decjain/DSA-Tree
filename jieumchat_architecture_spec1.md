...
## 2.1. Request Execution Sequence Diagram

The sequence diagram below illustrates the runtime interaction and message exchange between backend components when a user submits a query to a shared chatbot.
...
sequenceDiagram
    ...
    participant Lock as "Chat Lock Manager"
    participant PG as "PostgreSQL DB"
    participant Agent as "Agent Loop (run_query_agent)"
    participant LLM as "Qwen3 LLM API"
    participant WEAV as "Weaviate Vector DB"
    participant Rerank as "BGE-M3 Reranker"
    participant LF as "Langfuse Observability"

    Client->>QH: "POST /api/chatbots/{id}/query"
    activate QH
    Note over QH: "Extract User ID & Auth Headers"
    
    QH->>PG: "verify_access(chatbot_id, user_id)"
    activate PG
    PG-->>QH: "Access Granted + Data Source Scope"
    deactivate PG

    QH->>Lock: "acquire(chat_id)"
    activate Lock
    Note over Lock: "Prevents concurrent request collisions"
    
    QH->>Agent: "Initialize Agent Session"
    activate Agent
    
    rect rgb(240, 240, 255)
        Note over Agent, LLM: "Multi-Step Investigation Loop (Up to 12 turns)"
        
        Agent->>LLM: "Prompt + Chat History + Tools Schema"
        activate LLM
        LLM-->>Agent: "Tool Call: hybrid_search(query='Samsung SSO')"
        deactivate LLM
        
        Agent->>Agent: "Inject Dynamic ACLs based on Chatbot Scope"
        
        Agent->>WEAV: "GraphQL Hybrid Search (Vector + BM25 + ACL filters)"
        activate WEAV
        WEAV-->>Agent: "Matched Chunk Results"
        deactivate WEAV
        
        Agent->>Rerank: "Rerank Chunks (Score = Reranker^2.0 * Recency^0.6)"
        activate Rerank
        Rerank-->>Agent: "Sorted High-Relevance Chunks"
        deactivate Rerank
        
        Agent->>Client: "SSE Reaction Stream: 'mag' (Searching...)"
    end
    
    Agent->>LLM: "Synthesize Final Answer with Cited Evidence"
    activate LLM
    LLM-->>Agent: "Answer Text + Source Citations"
    deactivate LLM
    
    Agent->>Client: "SSE Answer Stream: text/event-stream (chunks)"
    
    Agent->>PG: "Save Dialog State & History Update"
    activate PG
    PG-->>Agent: "State Saved"
    deactivate PG

    Agent-->>QH: "Execution Finished"
    deactivate Agent

    QH->>Lock: "release(chat_id)"
    deactivate Lock

    QH->>LF: "Push Telemetry and Session Traces"
    activate LF
    LF-->>QH: "Trace Registered"
    deactivate LF

    QH->>Client: "SSE Event: 'end' [white_check_mark]"
    deactivate QH
```

## 2.2. System Data Flow Diagram (DFD)

The following Data Flow Diagram illustrates how document payloads, query tokens, and security metadata travel through processes (P1-P5), database stores (D1-D3), and external APIs.

```mermaid
graph TD
    %% External Entities (EE)
    User["[EE] User Client"]
    Atlassian["[EE] Atlassian APIs (Jira / Confluence)"]
    LLM_API["[EE] LLM API (Qwen3)"]
    Embed_API["[EE] Embedding API (Nomic)"]

    %% Data Stores
    D1[("(D1) PostgreSQL DB")]
    D2[("(D2) Weaviate Vector DB")]
    D3[("(D3) DialogState Cache")]

    %% Processes
    P1(("P1: Authenticate & Authorize Request"))
    P2(("P2: Discover & Extract Raw Source Data"))
    P3(("P3: Vectorize, Chunk & Store Documents"))
    P4(("P4: Orchestrate Investigation Agent Loop"))
    P5(("P5: Execute Hybrid Search & Context Synthesis"))

    %% Data Flows
    %% Ingestion Data Flow
    Atlassian -->|"1.1 Raw JSON/Markdown"| P2
    P2 -->|"1.2 Document metadata"| D1
    P2 -->|"1.3 StorageJob payload"| P3
    P3 -->|"1.4 Text chunks"| Embed_API
    Embed_API -->|"1.5 1536-dim Vectors"| P3
    P3 -->|"1.6 Vector indices & chunk records"| D2

    %% Query & Access Data Flow
    User -->|"2.1 Chat query + Chat ID"| P1
    P1 -->|"2.2 Read ACLs & PATs"| D1
    D1 -->|"2.3 User rights & source mapping"| P1
    P1 -->|"2.4 Authorized query context"| P4

    P4 -->|"3.1 Dialog history & state query"| D3
    D3 -->|"3.2 Prior turns summary"| P4

    P4 -->|"4.1 Formulate prompt"| LLM_API
    LLM_API -->|"4.2 Next Action / Tool Call"| P4

    P4 -->|"5.1 Search query with injected ACLs"| P5
    P5 -->|"5.2 Sparse & dense queries"| D2
    D2 -->|"5.3 Unranked chunk candidates"| P5
    P5 -->|"5.4 Reranked evidence items"| P4

    P4 -->|"6.1 Reranked context + Query"| LLM_API
    LLM_API -->|"6.2 Answer stream tokens"| P4
    P4 -->|"6.3 SSE Event Stream"| User
    P4 -->|"6.4 Updated dialog state"| D3

    %% Styling for DFD elements
    classDef ee fill:#fdf,stroke:#333,stroke-width:2px;
    classDef store fill:#ffd,stroke:#333,stroke-width:2px;
    classDef process fill:#dff,stroke:#333,stroke-width:2px;

    class User,Atlassian,LLM_API,Embed_API ee;
...
## 7. Critical Backend Topics to Master for the Interview

To successfully pitch your experience working on this architecture, master the following core backend engineering topics:

### 1. CPU vs. I/O Bound Concurrency (Python Multiprocessing + Asyncio)
*   **The Problem**: Python's Global Interpreter Lock (GIL) prevents CPU-bound threads (e.g., text parsing, token counting, and embedding preparation) from executing in parallel. Running both ingestion and querying in a single process degrades throughput.
*   **The Solution**: Jieumchat separates these bounds. The FastAPI server handles network-bound collection (`CollectionWorker` loops using `asyncio`), while spawning a completely separate OS subprocess (`StorageProcess` using `multiprocessing.get_context("spawn")`) to isolate embedding generation and batch gRPC writes to Weaviate. Communication is coordinated via a thread-safe `multiprocessing.Queue` (Bridge Pattern).
*   **Interview Focus**: Explain how you bypassed the GIL to maintain low query latency while running background bulk ingestion.

### 2. Proactive & Reactive Throttling (API Rate Limiting)
*   **The Problem**: External Atlassian (Jira/Confluence) APIs enforce strict rate limits (150 requests/min per Personal Access Token). Standard crawling triggers `HTTP 429 Too Many Requests` blockages, stalling the pipeline.
*   **The Solution**: A hybrid throttling approach:
    1.  **Proactive**: Parsing Atlassian's response headers (`x-ratelimit-remaining` and `x-ratelimit-reset`) to check remaining quotas and sleep workers before hitting limits.
    2.  **Reactive**: Wrapping client calls in retry decorators (`with_async_retry`) using exponential backoff with random jitter ($delay = base \times 2^{attempt} + jitter$).
*   **Interview Focus**: Discuss trade-offs between sync intervals (data freshness) and client rate limits, demonstrating how to protect downstream systems.

### 3. Dynamic Database Query Injection (Vector DB ACL Security)
*   **The Problem**: Document search permissions cannot be checked *after* vector retrieval (post-filtering) because that leads to "result starvation" or data leaks. Security must be checked *during* index lookup.
*   **The Solution**: User permissions are mapped in PostgreSQL (`data_source_access_grants`). At query time, the system resolves the caller's allowed data sources and intercepts the Weaviate search or raw GraphQL queries. It dynamically injects `data_source_uuid` filters directly into Weaviate's search filters (`inject_acl_filter`), enforcing access control at the database level.
*   **Interview Focus**: Detail how you designed the relational-to-vector boundary to ensure zero-privilege-escalation.

### 4. Agentic Guardrails & Context Compaction
*   **The Problem**: LLMs operating in multi-step tool-calling loops can get stuck in infinite execution loops (calling the same search tool repeatedly) and easily blow past prompt context windows.
*   **The Solution**: 
    1.  **Loop Guardrails**: Creating execution keys (`tool_name + arguments`) and caching them in memory during a single query session. If a duplicate call is detected, it overrides the LLM step with a mock error, forcing the agent to exit the loop.
    2.  **Context Compaction**: Monitoring prompt token counts against a soft threshold (e.g., 70% of the token limit). If exceeded, a compaction routine summarizes prior turns to free up context.
*   **Interview Focus**: Highlight how you built safety guardrails to control LLM costs and prevent system loops.

### 5. SSE (Server-Sent Events) & Session Lock Management
*   **The Problem**: Queries utilizing multi-turn agent search can take several seconds to complete. Standard HTTP requests timeout, while WebSockets introduce unnecessary handshake complexity. Multiple rapid clicks from a user can also cause database race conditions.
*   **The Solution**: The backend streams incremental tokens and intermediate statuses (reactions like `eyes`, `clipboard`, `mag`) using FastAPI's `StreamingResponse` (SSE, `text/event-stream`). To prevent session collisions, a `chat_request_lock_manager` acquires an async lock on the `chat_id` for the duration of the stream.
*   **Interview Focus**: Explain the request lifecycle and how to handle concurrent state mutations in a stateful chat session.
